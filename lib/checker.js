...
// Includes 参数限制 / 参数错误: Weibo returns these (sometimes with state:true!)
// when it rejects the request (bad/absent Referer, anti-bot, rate limit).
...
function interpret(email, httpStatus, text) {
  ...
  // Registered → the decisive signal the user specified. Check first.
  if (REGISTERED_RE.test(msg)) {
    return { ...base, status: 'registered', registered: true, message: msg, evidence };
  }

  // Captcha / rate-limit / parameter-restriction → do not guess.
  // MUST come before the state===true check: a rejected request can carry
  // state:true with msg "参数限制01", which is NOT "available".
