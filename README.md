...
- **持续 `inconclusive`（验证码 / 参数限制）：** 触发了微博风控 / 频率限制，结果不可靠。工具已内置自适应退避（连续受阻会自动放慢并重试），但仍应减少数量、降低频率或稍后再试。批量越大越容易被限速。页面提供「自动重试未确定项直到全部确定」开关与「每个间隔 / 轮间等待」设置，可挂机让其耐心重试到每个候选都有确定结论，从而不会把目标漏在未验证的一批里。
